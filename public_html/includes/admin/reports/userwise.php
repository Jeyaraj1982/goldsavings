                    ...
                    <div class="card-body" style="padding-bottom:0px">
                        <div class="row">
                            <div class="col-sm-12 mb-3">
                                ...
                                <input type="text" name="UserID" id="UserID" class="form-control" placeholder="User Name/Mobile Number">
                                <span id="ErrUserID" class="error_msg"></span>
                            </div>
                            <div class="col-sm-12 mb-3" id="UserResult">
                            </div>
                        </div>
                    </div>
                </div>
  ...
  inp.addEventListener("input", function(e) {
      ...
      /*for each item in the array...*/ 
       ...
       $.each(arr, function(i, item) {
        ...
        if ( (item.text.substr(0, val.length).toUpperCase() == val.toUpperCase()) ||  (item.MobileNumber.substr(0, val.length).toUpperCase() == val.toUpperCase()) ||  (item.UserID.substr(0, val.length).toUpperCase() == val.toUpperCase()) ) {
          /*create a DIV element for each matching element:*/
          b = document.createElement("DIV");
          /*make the matching letters bold:*/
          html = "<span class='row' style='background:none;'>";
            //html += "<span class='col-sm-2' style='background:none'>"+item.UserID+"</span>";
            html += "<span class='col-sm-4' style='background:none'>"+item.UserName+"</span>";
          ...
          b.addEventListener("click", function(e) {
              ...
              txtHtml = '<div class="row">'
                                    + '<div class="col-sm-4">'
                                        + '<label class="form-label" style="font-weight:bold;margin-bottom:0px !important">User Name</label><br>'
                                        +  item.UserName
                                    + '</div>'
                                    + '<div class="col-sm-4">'
                                    ...
                                    + '<div class="col-sm-12" style="text-align:right;">'
                                        ...
                                        + '<a href="'+URL+'dashboard.php?action=masters/users/view&user='+item.UserID+'" target="_blank" class="btn btn-outline-primary btn-sm">View Details</a>'
                                    + '</div>';
...
<?php 
$clients = $mysql->select("select UserID,Date(CreatedOn) as CreatedOn, UserID as value,UserName as text,AreaName, UserName, MobileNumber,concat(AddressLine1,', ',AddressLine2,', ',AreaName,', ',DistrictName,', ',StateName,', ',PinCode) as AddressLine from _tbl_masters_users where IsActive='1' order by UserName");
?>
autocomplete(document.getElementById("UserID"), <?php echo json_encode($clients);?>); 
