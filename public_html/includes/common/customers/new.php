...
function getAreaNames() {
    ...
    $.post(URL+ "webservice.php?action=listAllActive&method=AreaNames&DistrictNameID="+$('#DistrictNameID').val()+"&StateNameID="+$("#StateNameID").val(),"",function(data){
        ...
        if (obj.status=="success") {
            ...
            $("#AreaNameID").val(i);
           setTimeout(function(){
              $("#AreaNameID").select2({dropdownParent:$('#frm_create')});  
              $(".select2-container").each(function() {$(this).css({'z-index':'500','width':'calc(100% - 32px)'});}); 
            },1000);
           
          //  $('.select2-container').css({'z-index':'500 !important'});
...
function addNewBranch() {
    newbranch="";
    var param = $('#frm_create_branch').serialize();
   
    //clearDiv(['StateName','Remarks']);
    $.post(URL+"webservice.php?action=addNew&method=Branch",param,function(data){
        var obj = JSON.parse(data); 
        if (obj.status=="success") {
            newcustomercategory=$('#BranchName').val();
            $('#frm_create_branch').trigger("reset");
              openPopup();
               $('#newbranch').modal("hide");                           
            $('#popupcontent').html(success_content(obj.message,'closePopup();ListBranches')); 
        } else {
            if (obj.div!="") {
                $('#Err'+obj.div).html(obj.message);
                $('#process_popup').modal('hide');
