...
function loadContracts() {
    openPopup();
    $.post(URL+ "webservice.php?action=ListAllClosed&method=Contracts","",function(data){
        closePopup();
                                        ...
                                        + '<div class="col-6" style="text-align:right">'
                                             + '<div class="dropdown position-relative">'
                                                ...
                                                + '<div class="dropdown-menu dropdown-menu-end">'
                                                    + '<a class="dropdown-item" href="'+URL+'dashboard.php?action=contracts/viewclosedcontract&view='+data.ContractCode+'">View Contract</a>'
                                                    + '<a class="dropdown-item" href="'+URL+'dashboard.php?action=schemes/view&edit='+data.SchemeID+'&fpg=contracts/closed">View Scheme</a>';
                                                    if (parseInt(data.Receipts)>0) {
                                                        html +=  '<a class="dropdown-item" href="'+URL+'dashboard.php?action=receipts/receipt&contract='+data.ContractCode+'&fpg=contracts/closed"">View Receipts</a>';
                                                    } else {
                                                        html +=  '<a class="dropdown-item" href="javascript:void(0)" style="color:#aaa">View Receipts</a>';
                                                    }
                                                    if (data.VoucherNumber!="") {
                                                        html +=  '<a class="dropdown-item" href="'+URL+'dashboard.php?action=vouchers/viewvoucher&number='+data.VoucherNumber+'&fpg=contracts/closed"">View Voucher</a>';
                                                    } else {
