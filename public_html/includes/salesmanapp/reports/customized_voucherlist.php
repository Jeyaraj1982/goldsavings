<div class="container-fluid p-0 mb-3">
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-12">
                                <input class="form-check-input" type="checkbox" value="1" id="VoucherDate" name="VoucherDate"  onclick="printVoucherdate()">&nbsp;
                                Voucher Date
                                <div id="Voucherdate" style="display: none;">
                                    ...
                                    <span id="ErrVoucherDate" class="error_msg"></span> 
                                </div> 
                            </div>
                            <script>
                                function printVoucherdate() {
                                    OrderByContent();
                                    var checkBox = document.getElementById("VoucherDate");
                                    var div = document.getElementById("Voucherdate");
                            ...
                            <div class="col-12">
                                <input class="form-check-input" type="checkbox" value="1" id="VoucherType" name="VoucherType" onclick="OrderByContent()">&nbsp;
                                Voucher Number
                            </div>
                            <div class="col-12">
                            ...
                            <div class="col-12">
                                <input class="form-check-input" type="checkbox" value="1" id="GoldInGrams" name="GoldInGrams" onclick="OrderByContent()">&nbsp;
                                Gold 
                            </div>
                            <div class="col-12">
                            ...
                            <div class="col-12">
                                <input class="form-check-input" type="checkbox" value="1" id="VoucherStatus" name="VoucherStatus" onclick="printStatus()">&nbsp;
                                Status
                                <div style="display:none" id="selectVoucherStatus">
                                    <div class="input-group">
                                        <select class="form-select" name="selectVoucherStatusFilter" id="selectVoucherStatusFilter">
                            ...
                            <script>
                                function printStatus() {
                                    OrderByContent();
                                    var checkBox = document.getElementById("VoucherStatus");
                                    var div = document.getElementById("selectVoucherStatus");
                                    if (checkBox.checked == true){
                                        div.style.display = "block";
                                    } else {
                                        div.style.display = "none";
  ...
  flex: inherit;">>
       ...
       <div class="col-sm-12 mb-3" style="text-align:right;">
        <a href="<?php echo URL;?>dashboard.php" class="btn btn-outline-primary btn-sm" style="font-size: 10px">Back</a>&nbsp;&nbsp;
            <button type="button" onclick="getData()" class="btn btn-primary btn-sm" style="font-size: 10px">Get Report</button>    
    </div>  
    </div> 
...
function OrderByContent() {
     ...
     var html = "";
     if ($('#VoucherDate').prop("checked")) {
        html += "<option value='VoucherDate'>Voucher Date</option>";
     }
     ...
     if ($('#VoucherType').prop("checked")) {
        html += "<option value='VoucherType'>Voucher Type</option>";
     } 
     if ($('#ContractCode').prop("checked")) {
        html += "<option value='ContractCode'>Contract ID</option>";
     }
     if ($('#GoldInGrams').prop("checked")) {
        html += "<option value='GoldInGrams'>Gold</option>";
     }
    ...
    if ($('#VoucherStatus').prop("checked")) {
        html += "<option value='VoucherStatus'>Status</option>";
     }
        if (html==""){
...
function getData() {
    ...
    openPopup();
    clearDiv(['ReceiptDate','message']);
    $.post(URL+ "webservice.php?action=listCustomize&method=Vouchers",param,function(data){
        closePopup();
        ...
        if (obj.status=="success") {
            ...
            if ($('#GoldInGrams').prop("checked")) {
                header += "<th>Gold (Grams)</th>";
                column_count++;
            }
            if ($('#MaterialType').prop("checked")) {
                header += "<th>Material Type</th>";
                column_count++;
            }
            if ($('#TotalPaidAmount').prop("checked")) {
        ...
        header += "</tr>";
            ...
            $.each(obj.data, function (index, data) {
              ...
              if ($('#VoucherType').prop("checked")) {
               html += '<td>' + data.VoucherType + '</td>';
              }
              if ($('#ContractCode').prop("checked")) {
               html += '<td>' + data.ContractCode + '</td>';
              }
              if ($('#GoldInGrams').prop("checked")) {
               html += '<td>' + data.GoldInGrams + '</td>';
              }
              if ($('#MaterialType').prop("checked")) {
