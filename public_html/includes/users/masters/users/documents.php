...
var _userID='<?php echo $data[0]['UserID'];?>';

function listdocuments() {
    openPopup();
    $.post(URL+ "webservice.php?action=ListDocuments&method=Users&UserID=<?php echo $data[0]["UserID"];?>","",function(data){
        closePopup();
...
function addNew() {
  ...
  clearDiv(['DocumentTypes','DocumentFile','Remarks']);
    $.post(URL+"webservice.php?action=addDocument&method=Users",param,function(data){
        var obj = JSON.parse(data); 
        if (obj.status=="success") {
